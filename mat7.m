...
for object = 1:length(stats)
    ...
    result1 = insertShape(result1, 'Rectangle', bb, 'Color', 'green', 'LineWidth', 2);  
    % insertText(image, position, pos_value, characteristics, charact_value)
    % default -> BoxColor:yellow - FontColor: black
    result1 = insertText(result1, [bb(1,1)-15 bb(1,2)], object, 'BoxOpacity', 1, 'FontSize', 10, 'BoxColor', 'green');
    ...
    [M1] = improfile(data,x,y);
    
    % Consider only the pixels of red channel
    M2 = M1(:,:,1); 
    l2 = length(M2);
    j = 1;
    M3 = zeros(1);
    
    % Get only the pixels with value > 25
    for i = 1:l2
        if M2(i,1) > 25
            M3(j,1) = M2(i,1);
            j=j+1;
    ...
    tail = 0;
    for t = 1:length(M3)
        if or(M3(t,1) == max, M3(t,1)> (max-25))
                tail = 0;
        end;
        tail = tail+1;
    end;
    ...
    crop = imcrop(result1,bb);
    imshow(crop);
    title({['Comet ',num2str(object)],['Length of tail ',num2str(tail)]});
...
result = insertText(result1, [bw.ImageSize(1)/2 5], nComet, 'BoxOpacity', 1, 'FontSize', 10);
figure, imshow(result), title('Comets');